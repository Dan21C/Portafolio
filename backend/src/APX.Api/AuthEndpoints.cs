...
{
    public static IEndpointRouteBuilder MapAuthApi(this IEndpointRouteBuilder endpoints, AuthOptions options)
    {
        ...
        auth.MapPost("/otp/verify", async (VerifyOtpDto request, HttpContext http, AuthService service, IWebHostEnvironment environment, CancellationToken ct) =>
        {
            ...
            var created = result.Value!; http.Response.Cookies.Append(options.CookieName, created.Token, Cookie(environment, created.Session.ExpiresAt)); return Results.Ok(created.Session);
        ...
        {
            await service.LogoutAsync(http.Request.Cookies[options.CookieName], Context(http), ct); http.Response.Cookies.Delete(options.CookieName, Cookie(environment, DateTimeOffset.UnixEpoch)); return Results.NoContent();
    ...
    private static CookieOptions Cookie(IWebHostEnvironment environment, DateTimeOffset expires) => new() { HttpOnly = true, Secure = !environment.IsDevelopment(), SameSite = SameSiteMode.Lax, Path = "/", Expires = expires, IsEssential = true };
}
