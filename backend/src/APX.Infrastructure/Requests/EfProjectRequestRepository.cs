...
public sealed class EfProjectRequestRepository(ApxDbContext db) : IProjectRequestRepository
{
    ...
    public Task<ProjectRequestNotificationDto?> GetNotificationAsync(Guid id, CancellationToken ct) => db.ProjectRequests.AsNoTracking().Where(x => x.Id == id).Select(x => new ProjectRequestNotificationDto(x.Id, x.RequestNumber, x.Name, x.Company, x.Email, x.Phone, x.City, x.ApproximateDate, x.Attendees, x.Message, x.CreatedAt, x.Items.OrderBy(i => i.SolutionName).Select(i => i.SolutionName).ToList())).SingleOrDefaultAsync(ct);
