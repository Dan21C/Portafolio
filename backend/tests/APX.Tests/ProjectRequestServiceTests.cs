...
public sealed class ProjectRequestServiceTests
{
    ...
    private static ProjectRequestService Service(FakeRepository? repo = null) => new(repo ?? new(), new FakeNotifier(), new(20, "2026-08"));
    ...
    private sealed class FakeRepository : IProjectRequestRepository
    {
        ...
        public Task<ProjectRequestNotificationDto?> GetNotificationAsync(Guid id, CancellationToken ct) => Task.FromResult<ProjectRequestNotificationDto?>(new(id, "APX-000001", "Ada", null, "ada@example.com", "300", "Bogotá", null, null, null, DateTimeOffset.UtcNow, ["Solution"]));
    }
    private sealed class FakeNotifier : IProjectRequestNotifier { public Task NotifyCreatedAsync(ProjectRequestNotificationDto request, CancellationToken ct) => Task.CompletedTask; }
}
